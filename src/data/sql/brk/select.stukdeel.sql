...
WITH stukdelen AS (
    SELECT sdl.identificatie                                 AS brk_sdl_id
         ...
         , CASE
               ...
               WHEN
                   tng.einddatum IS NULL OR
                   asg.zrt_einddatum IS NULL OR
                   akt.expiration_date IS NULL OR
                   art.expiration_date IS NULL
               THEN
                   NULL
               ELSE
                   GREATEST(tng.einddatum, asg.zrt_begindatum, akt.expiration_date, art.expiration_date)
    ...
    LEFT JOIN (
        SELECT tip.stukdeel_identificatie
             ...
             , CASE
                 ...
                 WHEN
                     SUM(CASE WHEN tng.einddatum IS NULL THEN 0 ELSE 1 END) < SUM(1)
                 THEN
                     NULL
                 ELSE
                     MAX(tng.einddatum)
             ...
             , array_to_json(
                   array_agg(
                       ...
                       ) ORDER BY tng.brk_tng_id, tng.nrn_tng_id)
                   )                                                                   AS tng_ids
...
--
    LEFT JOIN (
        SELECT stukdeel_identificatie
             ...
             , CASE
                 WHEN
                     SUM(CASE WHEN expiration_date IS NULL THEN 0 ELSE 1 END) < SUM(1)
                 THEN
                     NULL
                 ELSE
                     MAX(expiration_date)
        ...
        FROM (
            SELECT aip.stukdeel_identificatie
                 , CASE
                     WHEN
                         SUM(CASE WHEN art.expiration_date IS NULL THEN 0 ELSE 1 END) < SUM(1)
                     THEN
                         NULL
                     ELSE
                         MAX(art.expiration_date)
                     END                                                               AS expiration_date
                , atg.identificatie
                , atg.id
    ...
    INNER JOIN (
        ...
        FROM (
            SELECT aip.stukdeel_identificatie
                 , CASE
                     WHEN
                         SUM(CASE WHEN akt.expiration_date IS NULL THEN 0 ELSE 1 END) < SUM(1)
                     THEN
                         NULL
                     ELSE
                         MAX(akt.expiration_date)
                     END                                                               AS expiration_date
                 , atg.identificatie
                 , atg.id
            FROM brk.aantekeningisgebaseerdop aip
    ...
    LEFT JOIN (
        SELECT stukdeel_identificatie
             ...
             , CASE
                 ...
                 WHEN
                     SUM(CASE WHEN zrt_einddatum IS NULL THEN 0 ELSE 1 END) < SUM(1)
                 THEN
                     NULL
                 ELSE
                     MAX(zrt_einddatum)
             ...
             , array_to_json(
                 array_agg(
                     json_build_object('brk_zrt_id', identificatie)
        ...
        FROM (
            SELECT arl.stukdeel_identificatie
                 ...
                 , CASE
                     -- NULL if one of the zrt_einddatums is NULL, otherwise max
                     WHEN
                         SUM(CASE WHEN zrt.zrt_einddatum IS NULL THEN 0 ELSE 1 END) < SUM(1)
                     THEN
                         NULL
                     ELSE
                         MAX(zrt.zrt_einddatum)
                     END                                                               AS zrt_einddatum
                 , zrt.identificatie
        ...
        ON (asg.stukdeel_identificatie = sdl.identificatie)
    --
...
JOIN (
    ...
    ON ordering.nrn_sdl_id = sdl.nrn_sdl_id
--
