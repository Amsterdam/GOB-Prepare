...
FROM (
    SELECT
        ...
        array_to_json(
                array_agg(
                        json_build_object(
                                'brk_kot_id', ontst_uit_brk_kot_id
                            ) ORDER BY
                            ontst_uit_brk_kot_id
                    )
            ) AS ontstaan_uit_kadastraalobject
    FROM (
         SELECT kot.nrn_kot_id,
                kot.nrn_kot_volgnr,
                ontst_uit_kot.brk_kot_id AS ontst_uit_brk_kot_id
         ...
         GROUP BY kot.nrn_kot_id, kot.nrn_kot_volgnr, ontst_uit_kot.brk_kot_id
    ) qq
        ...
        FROM (
            SELECT
                ...
                array_to_json(
                        array_agg(
                                json_build_object(
                                        'brk_kot_id', gperc_brk_kot_id
                                    ) ORDER BY gperc_brk_kot_id
                            )
                    ) AS relatie_g_perceel
            FROM (
                 SELECT kot.nrn_kot_id,
                        kot.nrn_kot_volgnr,
                        gperceel.brk_kot_id AS gperc_brk_kot_id
                 FROM brk_prep.kadastraal_object kot
                          LEFT JOIN jsonb_array_elements(kot.ontstaan_uit_kadastraalobject) json_elms(obj)
                                    ON TRUE
                          LEFT JOIN brk_prep.kadastraal_object ontst_uit_kot
                 ...
                 WHERE ontst_uit_kot.relatie_g_perceel <> 'null'
                   ...
                   AND kot.relatie_g_perceel = 'null'
                   AND kot.ontstaan_uit_kadastraalobject <> 'null'
                 GROUP BY kot.nrn_kot_id, kot.nrn_kot_volgnr, gperc_brk_kot_id
            ) qq
            GROUP BY nrn_kot_id, nrn_kot_volgnr
        ) q(nrn_kot_id, nrn_kot_volgnr, relatie_g_perceel)
        WHERE kot.nrn_kot_id=q.nrn_kot_id
