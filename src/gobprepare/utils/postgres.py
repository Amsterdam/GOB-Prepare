...
@cache
def _is_columnar_supported(postgres_datastore: PostgresDatastore):
    result = int(postgres_datastore.get_version().split('.')[0]) >= 12 and \
             postgres_datastore.is_extension_enabled("citus")

    if not result:
        logger.warning("Columnar tables are not supported by this Postgres instance. "
                       "Need version >= 12 with 'citus' enabled.")
    return result


def create_table_columnar_as_query(postgres_datastore: PostgresDatastore, tablename: str, as_query: str):
    """Return the CREATE TABLE query for Postgres for a CREATE TABLE tablename AS as_query query

    Creates a columnar table if supported by this Postgres instance.
    """
...
def create_table_columnar_query(postgres_datastore: PostgresDatastore, tablename: str, columndefs: str):
